...
if(isset($_POST['add'])){
    ...
    if($error === 4){
        ...
        content='1; url=?preview=gallery_edit'>";
        return false;
    }
    ...
    if(!in_array($image_extend, $image_extend_valid)){
        ...
        echo "<meta http-equiv='refresh'
        content='1; url=?preview=gallery_add'>";
        return false;
    }
    ...
    $uploaded = move_uploaded_file($image_location,"../image/gallery/$image_name");
    if($uploaded){  
        mysqli_query($conn, "INSERT INTO blog_gallery SET 
            title = '$title',
            date = '$date', 
            image = '$image_name'
            ");
